...
describe('Chat Module', function () {
  ...
  it('should be able to create a new Chatroom', function(){
    ...
    fs.existsSync('./data/secret.json', function (exists){
      expect( exists, 'secret.json files does not exist'  ).to.be.true;
    });
  });
  ...
  it('should get a user\'s messages from all chatrooms', function () {
    ...
    messages = chatModule.postMessage(message1, 'secret');
    expect( chatModule.getUserMessages('Alice')).to.have.length( 2 );

    messages = chatModule.postMessage(message1, 'tmpchatroom');
